...
const Cart = () => {
  const { products, currency, cartItems, updateQuantity, navigate, getStock } = useContext(ShopContext);
      ...
      <div>
        {
          cartData.map((item, index) => {
            ...
            const stockLeft = getStock(item._id, item.size)

            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                <div className='flex items-start gap-6'>
                  <img className='w-16 sm:w-20' src={imgUrl(productData.image[0])} alt="" />
                  <div>
                    ...
                    {stockLeft < item.quantity && (
                      <p className='text-xs text-red-500 mt-1'>Only {stockLeft} left — reduce quantity</p>
                    )}
                  </div>
                </div>
