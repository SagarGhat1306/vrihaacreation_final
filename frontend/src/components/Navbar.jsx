...
const Navbar = () => {
    ...
    const { setShowSearch, GetcartCount, navigate, setToken, setCartItems, token, categories } = useContext(ShopContext)
    ...
    return (
        <div className='flex items-center justify-between py-5 font-medium'>
            ...
            <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
                ...
                <NavLink to='/become-seller' className='flex flex-col items-center gap-1'>
                    <p>BECOME A SELLER</p>
                    <hr className='w-2/3 border-none h-[1.5px] bg-gray-700 hidden'></hr>
                </NavLink>
            </ul>
            ...
            <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all z-50 ${visible ? 'w-full' : 'w-0'}`}>
                <div className='flex flex-col text-gray-600'>
                    ...
                    <NavLink onClick={() => setvisible(false)} className="py-2 pl-6  border" to="/become-seller">BECOME A SELLER</NavLink>

                    {/* all categories in mobile menu — Meesho style */}
                    <p className='py-2 pl-6 border bg-gray-50 font-medium'>CATEGORIES</p>
                    {categories.map((cat) => (
                        <NavLink
                            key={cat._id}
                            onClick={() => setvisible(false)}
                            className="py-2 pl-10 border text-sm"
